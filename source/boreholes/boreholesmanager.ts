...
export class BoreholesManager {
   ...
   parse() {
      ...
      return this.boreholes.load().then(data => {
         ...
         let holes = data.filter(hole => hole.lon > bbox[0] && hole.lon <= bbox[2] && hole.lat > bbox[1] && hole.lat <= bbox[3]);

         if (this.options.elevationLookup) {
            let coords = holes.map(hole => proj4("EPSG:4283", "EPSG:3857", [hole.lon, hole.lat, hole.elevation]));
            return this.options.elevationLookup(coords).then(elevations => {
